...
_TOC_NAVIGATE_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "Вызывай без clause чтобы увидеть содержание с верхнего уровня, затем укажи clause конкретного "
            "пункта, чтобы раскрыть его вложенные подразделы. depth — сколько уровней вложенности "
            "показать сразу (по умолчанию 2: разделы и подразделы). "
...
План должен сопровождаться немедленным вызовом инструмента в том же ответе. Навигацией по содержанию (toc_navigate) найди нужные разделы, загрузи их страницы (get_pages_by_clause, get_page_markdown) и на основании полученного содержимого сверь черновик и собери финальный ответ. В следующих итерациях повторный план не нужен — просто продолжай выполнение.
